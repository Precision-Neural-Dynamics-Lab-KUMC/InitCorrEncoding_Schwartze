...
ph = pie(mean_R2_partial_init([2,1,3]), {['Vel: ' num2str(pievelinit, '%2.1f') '%'],[ 'Pos: ' num2str(pieposinit, '%2.1f') '%'],[ 'Speed: ' num2str(piespinit, '%2.1f') '%']});
ph(2).FontSize = 14;  ph(4).FontSize = 14;  ph(6).FontSize = 14;
colormap([vel_color; pos_color; speed_color])
ph(4).Position(1:2) = [-0.9828    0.3435];  % ph(4).Position(2)+0.05;
ph(6).Position(1:2) = [-1.0058   -0.0459]; %ph(6).Position(2)-0.05;
% set(gcf,'Position',[0 0 600 400]);
% print(gcf, ['./Schwartze_Paper1/figure_graphics/' monkey '_figure3_violin_init_p'], '-dpng')  
% print(gcf, ['./figure_graphics/' monkey '_figure3_violin_init_p_test'], '-painters', '-dpdf')  
...
ph(2).FontSize = 14;  ph(4).FontSize = 14;  ph(6).FontSize = 14;
%Pie chart labels manually adjusted
colormap([vel_color; pos_color; speed_color])
% set(gcf,'Position',[0 0 600 400]);
